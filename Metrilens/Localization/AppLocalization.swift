...
enum AppTextCatalog {
    ...
    private static let englishOverrides: [String: String] = [
        ...
        "preferences.halfSecond": "0.5 sec",
        "preferences.seconds": "%d sec",
        "preferences.sourceInformation": "Temperature: Metrilens reads AppleSmartBattery battery temperature only. It separates the resettable session maximum from the device lifetime maximum. Precise CPU/GPU temperature is outside the current scope.\n\nMemory: “Memory Usage” is internal + wired + compressor memory. This stable product definition may differ from Activity Monitor.",
        "settings.highBatteryTemperature": "High Battery Temperature",
        "sparkline.accessibilityLabel": "%@ usage over the last 10 minutes",
        "sparkline.summaryAccessibility": ", last 60 seconds average %.1f%%, peak %.1f%%",
        "summary.available": "10 min chart · 60 sec avg %.1f%% · Peak %.1f%%",
        "summary.unavailable": "10 min chart · 60 sec avg — · Peak —",
    ...
    private static let chineseTranslations: [String: String] = [
        ...
        "preferences.halfSecond": "0.5 秒",
        "preferences.seconds": "%d 秒",
        "preferences.sourceInformation": "温度说明：只读取 AppleSmartBattery 电池温度，并区分可重置的本次最高与设备历史最高。CPU/GPU 精确温度不在当前范围内。\n\n内存说明：“内存占用”由 internal、wired 与 compressor 内存组成；这是稳定的产品口径，不承诺与活动监视器完全一致。",
        "settings.highBatteryTemperature": "电池温度过高",
        "sparkline.accessibilityLabel": "最近 10 分钟%@使用率",
        "sparkline.summaryAccessibility": "，最近 60 秒平均 %.1f%%，峰值 %.1f%%",
        "summary.available": "10 分钟曲线 · 近 60 秒平均 %.1f%% · 峰值 %.1f%%",
        "summary.unavailable": "10 分钟曲线 · 近 60 秒平均 — · 峰值 —",
