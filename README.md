...
- CPU 总占用、最近 60 秒折线、平均值和峰值
- “Metrilens 占用”口径的内存使用量、最近 60 秒折线、平均值和峰值
...
菜单栏可显示单个指标或自选的紧凑指标组合。界面支持跟随系统、简体中文和 English；温度及系统热状态按严重程度使用系统语义色。CPU、内存持续超阈值和严重系统热状态可选择发送本地通知，默认关闭，只有开启时才请求通知权限；应用位于前台时也会显示横幅并播放提示音。
...
登录启动、界面语言、菜单栏显示模式与指标、刷新频率、CPU/内存折线和本地提醒均可在分组设置中调整。设置损坏时 App 会自动恢复安全默认值；也可在设置窗口底部选择“恢复默认设置…”。
