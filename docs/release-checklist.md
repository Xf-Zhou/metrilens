...
- [ ] 电池电量、供电状态和循环次数与系统信息合理一致；系统提供可识别健康状态时正确显示，否则健康状态行隐藏。
...
- [ ] 单项/紧凑菜单栏模式、指标排序、分隔符、精度和中英文标签切换正常；网络速率横排/竖排、系统原生/深海监测/机舱琥珀样式可切换并持久化；CPU/内存可选择 0.5、1、2、5、10、30 秒刷新频率。
- [ ] 菜单栏选中时文字跟随系统高亮色；取消选中后告警、过期和正常颜色恢复正确。
