...
- 弹窗提供系统原生、深海监测和机舱琥珀三套可持久化的样式；两套自定义样式使用深色监测面板与独立曲线强调色，但不覆盖告警语义色。
- CPU 与内存折线各自保存最近 10 分钟、最多 601 个样本；平均值、峰值和发热诊断仍只使用最近 60 秒样本。
...
- 电池详情读取电量、充电/外接电源状态、循环次数和系统提供的健康状态。空的 `BatteryHealthCondition` 必须继续回退读取 `BatteryHealth`；只展示已识别的健康状态，系统未提供或返回未知值时隐藏该行。电池详情与温度共用 10/30/120 秒调度，但温度字段不支持时仍可继续读取其他电池信息。
...
- 系统原生样式使用 `NSPopover`；深海监测和机舱琥珀使用自绘无边框面板，确保圆角与箭头完全采用主题背景。
...
- 低电量模式下，CPU/内存有效采样周期固定为 `max(用户设置, 5 秒)`，即 1 秒或 2 秒配置会降为 5 秒，5、10、30 秒配置保持不变。
...
- 使用等宽数字，避免数值变化造成宽度跳动。
- 中文标签使用“内存”“电池”“网络”“磁盘余”，英文使用 `RAM`、`Batt`、`Net`、`Free`；CPU 在两种语言中均保留行业通用缩写。
- 颜色默认跟随系统菜单栏；菜单栏被选中时使用 macOS 原生高亮文本色，仅在未选中且确有告警时使用柔和的语义色，stale 状态使用次要色。
- 不使用持续动画。
...
- 菜单栏主指标
- 弹窗预设样式：系统原生、深海监测、机舱琥珀
- CPU/内存刷新间隔：1、2、5、10、30 秒
- 登录时启动
- 是否显示 CPU 微型折线
- 数据来源与温度能力说明
...
- 默认每 1 秒采样；用户选择 2、5、10 或 30 秒时同步调整。
...
- UI 主值命名为“内存占用”，使用 `usedBytes / totalBytes`；`purgeableBytes` 不进入主卡片。
...
└── MetricSampler
    ...
    ├── MetricHistorySeries（CPU、内存各一个；各含折线与统计缓冲）
    └── SystemSnapshot
            │
            ▼
      主线程 UI、提醒与诊断报告
...
| --- | --- | --- | --- | --- | --- |
| 弹窗打开 | 用户设置的 1/2/5/10/30 秒 | 用户设置的 1/2/5/10/30 秒 | 10 秒 | 用户设置的 1/2/5/10/30 秒 | 60 秒 |
...
- CPU 与内存分别维护最近 10 分钟、最多 601 点的折线缓冲，以及最近 60 秒、最多 64 点的统计缓冲；暂停或有效周期变化时同时清空，避免把不连续区间误画成连续历史。
- 隐藏状态停止图层重绘；模型变更也不能触发隐藏图表布局。
- 唤醒和实际电源来源变化属于强制刷新，可以越过普通 UI cooldown，但仍受 2 秒事件合并窗口和单 in-flight 约束。

### 5.2 CPU 与内存历史缓冲

- CPU 与内存分别持有一个 `MetricHistorySeries`，每个 series 内含独立的折线与统计缓冲，两项指标之间不共享样本或状态。
- 只保存带 monotonic timestamp 的有效 CPU/内存样本。
- 每次写入和读取时，折线缓冲删除早于当前时间 10 分钟的样本，统计缓冲删除早于 60 秒的样本。
- 折线缓冲容量上限为 601 个点，统计缓冲上限为 64 个点；达到上限时覆盖最旧点，不发生持续分配。
- 1/2/5/10/30 秒周期的折线分别最多展示约 601/301/121/61/21 个点，时间跨度始终按 timestamp 解释，而不是按点数推断。
...
- 电池详情的电量、供电状态、循环次数、健康度、空 `BatteryHealthCondition` 回退、未知健康值隐藏、字段缺失与无电池降级。
...
- CPU 折线缓冲的 10 分钟裁剪、601 点上限，60 秒统计口径，以及 1/2/5/10/30 秒切换。
...
- 低电量模式
- 睡眠与唤醒
- 菜单栏切换不同主指标、中英文标签、选中状态与告警/过期颜色转换
