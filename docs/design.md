...
上述扩展不引入长期历史数据库、遥测、进程扫描、外部采集进程或后台网络访问。提醒和更多菜单栏指标会增加相应的 CPU/内存采样，界面需明确由用户主动启用。
...
| UI/系统状态 | CPU | 内存 | 电池温度 |
| --- | --- | --- | --- |
| 弹窗打开 | 用户设置的 1/2/5 秒 | 用户设置的 1/2/5 秒 | 10 秒 |
| 弹窗关闭，主指标 CPU | 用户设置的 1/2/5 秒 | 暂停 | 暂停 |
| 弹窗关闭，主指标 MEM | 暂停 | 用户设置的 1/2/5 秒 | 暂停 |
| 弹窗关闭，主指标 BAT | 暂停 | 暂停 | 30 秒 |
| 低电量模式 | 活跃项至少 5 秒 | 活跃项至少 5 秒 | 活跃项 120 秒，否则暂停 |
| 睡眠 | 停止 | 停止 | 停止 |
...
- scheduler 拒绝早于有效周期的重复 CPU 样本；UI 事件不能通过快速切换向缓冲写入额外点。
...
| 当前会话缺少已验证字段 | `unsupported(.fieldMissing)`；只在唤醒或硬件拓扑变化后重试能力探测 |
...
- 读取失败时不能用 `0` 代替真实数据。
- stale 状态使用次要颜色并显示最后更新时间；TTL 到期后显示 `—`，不能继续保留旧数值。
...
每个阶段单独提交；下一阶段只能在本阶段命令和完成条件通过后开始。没有数据库或持久化迁移，阶段回滚边界为撤销该阶段提交。
